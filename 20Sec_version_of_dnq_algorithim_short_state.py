...
class Reward:
  ...
  def intermediate_reward(self, state, action, previous_action_list, orbital_characteristic,flight_angle_val):
        present_reward = 0
        ...
        height = state[5]
        height_reward = max(0.001 * (234000-height) / 2000, 0.0001)  # max reward 0.05
        # present_reward += height_reward

        if flight_angle_val >= 0:

            flight_angle_val = 1 + math.degrees(flight_angle_val)
            present_reward += 0.01 / (flight_angle_val ** 2) # Run 1
        else:
            flight_angle_val = -1 + math.degrees(flight_angle_val)
            present_reward += 0.01 / (flight_angle_val ** 2) # Run 1
        ...
        eccentricity_reward = 0.01 / (eccentricity_value ** 2)
        # present_reward += eccentricity_reward

        if action == previous_action_list[-1]:
            present_reward += 0.00
            if action != 0:
                present_reward += 0.001
        elif action == 1:
            present_reward += 0.00
        else:
            self.change_amount  += 1
            if self.change_amount > 6: # 3 for Run 1
                present_reward -= 0.003 * (self.change_amount - 6)
  ...
  def final_reward(self, orbital_characteristic, state):
        ...
        if perigee < 100000:
            present_reward -= 600  # adjusted penalty

        # Reward for average height
        height_reward = (perigee -227000) * 50 / 8000  # max reward 50
        present_reward += height_reward
...
class DQNagent:
    def __init__(self, state_size= 7, action_size=3, hidden_size=128, 
                 learning_rate=1e-4, gamma=1, buffer_size=50000, batch_size=64, model = None, optimizer_state_dict = None):
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        self.memory.push(state, action, reward, next_state, done) #make a reward class bc it's getting it from the environment
...
def train(agent, n_episodes = 1000, eps_start= 1, eps_end=0.01, eps_decay=0.92, target_update=10, used_saved_data_rate = 10, action_rate = 10, run_train_time = 100):
    '''
    Train a DQN agent.
    ...
    scores = []
    perigee_list  =  [orbital_param_data[0]]

    ecen_list = [orbital_param_data[2]]
    scores_window = deque(maxlen=100)
    eps = eps_start
    previous_action_list = 9*[3]
    action_angle = math.radians(90)
    average_end_values = []
    run_saved_value = True
    ...
    try:
        for i_episode in range(1, n_episodes + 1):
            previous_action_list = 9*[1]


            start_value = 4560 - run_train_time
            ...
            for time in range(start_value + 1, 4562):  # this just how much iteration we are going to do for now. This is 456.1 seconds, the rocket completes the burn at 456 secondd
                gravity_turn = action_angle != math.pi/2 # if not equal to 90 degrees
                present_pitch_angle = round(env.get_pitch_angle(), 4)

                if time % action_rate == 1 or action_rate == 1:
                    ...
                    done = time >= 4561 or next_state[5] < 0


                    # find the reward
                    if done:
                        # print(time)
                        print(score.get_reward())
                        reward = score.final_reward(present_orbital_param, next_state)
                        print(present_orbital_param)

                    else:
                        reward = score.intermediate_reward(next_state, action, previous_action_list, present_orbital_param, flight_angle_val) #I'm just going to hard code this for now

                    # Update the agent, state and score
                    agent.step(state, action, reward, next_state, done)
                    state = next_state
                    previous_action_list = previous_action_list[1:] + [action]
            ...
            # At the end of episode append and save scores
            scores_window.append(score.get_reward())
            scores.append(score.get_reward())
            perigee_list.append(present_orbital_param[0])
            ecen_list.append(present_orbital_param[2])

            eps = max(eps_end, eps_decay * eps)

            # Decrease epsilon
            # stop_val = 50
            # if len(average_end_values) >= 4:
            #     stop_val = sum(average_end_values) / len(average_end_values)
            #     average_end_values = average_end_values[1:]
            # else:
            #     stop_val = 50
            ...
            # if score.get_reward() > max(50, stop_val):
            #     eps = max(eps_end, eps_decay * eps)
            #     average_end_values = average_end_values[:-1]
            # else:
            #     print("Stop Limit:", max(50, stop_val), average_end_values, stop_val)
            #     # eps = min((eps/(eps_decay ** 15)), 1, eps)
            #     eps = 1
            #     run_saved_value = True

            # average_end_values.append(score.get_reward())
            # Print some info
            print(f"\rEpisode {i_episode}\tAverage Score: {score.get_reward():.2f}")
            ...
            if i_episode % 1000 == 0:
                ...
                model_file = {'model': DQN(),
                    'state_dict': agent.q_network.state_dict(),
                    'optimizer' : agent.optimizer.state_dict()
                    }

                torch.save(model_file, 'model20-072315SH_' + str(i_episode) + '.pth')
    ...
    env.display_data()
    plot_orbital_param(perigee_list, ecen_list)
    plot_reward_change(scores)
...
agent = DQNagent(learning_rate=1e-7)
# agent = DQNagent(1, 3)

# Train it
scores = train(agent,
               used_saved_data_rate = 10, 
               n_episodes = 400, run_train_time = 200, action_rate = 1, eps_end=0.005, 
            #    eps_decay=0.7
               )
...
torch.save(model_file, 'model20-072315SH.pth')