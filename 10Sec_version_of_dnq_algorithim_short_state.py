...
class Reward:
  ...
  def intermediate_reward(self, state, action, previous_action_list, orbital_characteristic,flight_angle_val):
        ...
        if action == previous_action_list[-1]:
            ...
            if self.change_amount > 3: # 3 for Run 1
                present_reward -= 0.003 * (self.change_amount - 3)
  ...
  def final_reward(self, orbital_characteristic, state):
        ...
        height_reward = (perigee -227000) * 50 / 3000  # max reward 50
        present_reward += height_reward
...
class DQNagent:
    def __init__(self, state_size= 7, action_size=3, hidden_size=128, 
                 learning_rate=1e-4, gamma=0.99, buffer_size=50000, batch_size=64, model = None, optimizer_state_dict = None):
...
def train(agent, n_episodes = 1000, eps_start= 1, eps_end=0.01, eps_decay=0.92, target_update=10, used_saved_data_rate = 10, action_rate = 10, run_train_time = 100):
    ...
    for i_episode in range(1, n_episodes + 1):
        ...
        if i_episode % 1000 == 0:
            ...
            torch.save(model_file, 'model072313SH_' + str(i_episode) + '.pth')
...
scores = train(agent,
               ...
               n_episodes = 400, run_train_time = 100, action_rate = 1, eps_end=0.005, 
            #    eps_decay=0.7
               )
...
torch.save(model_file, 'model072313SH.pth')